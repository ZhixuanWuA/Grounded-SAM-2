...
def process_videos(base_video_dir, output_dir, qa_json_path, sam2_checkpoint, model_cfg, grounding_model_id):
    # Load the qa.json data
    with open(qa_json_path, "r") as f:
        qa_data = json.load(f)

    # List of words to exclude
    question_words = ["what", "who", "which", "where", "when", "why", "how many"]
    ...
    sam2_image_model = build_sam2(model_cfg, sam2_checkpoint, device="cuda:0")
    image_predictor = SAM2ImagePredictor(sam2_image_model)

    processor = AutoProcessor.from_pretrained(grounding_model_id)
    grounding_model = AutoModelForZeroShotObjectDetection.from_pretrained(grounding_model_id).to("cuda:0")
    ...
    for video_dir in video_folders:
        ...
        matching_questions = [
            ...
            for item in qa_data if item["video_name"] == video_name
        ]

        if matching_questions:
            for question_id, input_question in matching_questions:
                ...
                nouns_and_pronouns = [
                    ...
                    if token.pos_ in ["NOUN", "PRON"] and token.text.lower() not in question_words
                ]
                ...
                for start_frame_idx in range(0, len(frame_names), step):
                    ...
                    inputs = processor(images=image, text=text, return_tensors="pt").to("cuda:0")
                    ...
                    if mask_dict.promote_type == "mask":
                        mask_dict.add_new_frame_annotation(mask_list=torch.tensor(masks).to("cuda:0"), box_list=torch.tensor(input_boxes), label_list=OBJECTS)
                    else:               
...
if __name__ == "__main__":
    # base_video_dir = "/home/hanjiale/wzx/Detect/Grounded-SAM-2-main/test/videos"
    # output_dir = "/home/hanjiale/wzx/Detect/Grounded-SAM-2-main/test/output"
    base_video_dir = "/home/zhangshaoxing/cv/datasets/MSVD_QA/framesall/frames"
    output_dir = "/home/zhangshaoxing/cv/datasets/MSVD_QA/test"
    qa_json_path = "/home/zhangshaoxing/cv/datasets/MSVD_QA/test_q.json"
    ...
    process_videos(base_video_dir, output_dir, qa_json_path, sam2_checkpoint, model_cfg, grounding_model_id)
